...
export class ShowBuildingAirQuality
    implements UseCase<ShowBuildingAirQualityInput>
{
    ...
    public async execute(input: ShowBuildingAirQualityInput): Promise<void> {
        ...
        const { enviroScore, roomScores } =
            await this._airQualityCalculator.calculateBuildingMetrics(
                buildingEntity
            );
