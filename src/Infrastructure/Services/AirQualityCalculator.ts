...
export class AirQualityCalculator {
    ...
    private aggregateAirData(airData: AirData, deviceData: any): void {
        airData.temperature =
            (airData.temperature ?? 0) + deviceData.temperature;
        ...
        if (deviceCount > 0) {
            airData.temperature =
                airData.temperature !== null
                    ? parseFloat((airData.temperature / deviceCount).toFixed(2))
                    : null;
            airData.humidity =
                airData.humidity !== null
                    ? parseFloat((airData.humidity / deviceCount).toFixed(2))
                    : null;
            airData.ppm =
                airData.ppm !== null
                    ? parseFloat((airData.ppm / deviceCount).toFixed(2))
                    : null;
        } else {
            airData.temperature = null;
            airData.humidity = null;
    ...
    private computeFinalEnviroScore(
        ...
        return deviceCount > 0
            ? Number(parseFloat((totalScore / deviceCount).toFixed(1)))
            : null;
    }
}
